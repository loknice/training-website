...
import { Fox } from '../src/models/fox';
...
// Тести API вебдодатку сайту про лисиців
describe('API вебдодатку сайту про лисиців', () => {
    ...
    const testMongoURI = MONGODB_URI.replace(/\/[^/]*$/, '/foxs-test');
    ...
    after(async () => {
        try {
            ...
            console.log('Тестову базу даних "foxs-test" успішно видалено');
        } catch (error) {
    ...
    // Перед кожним тестом очищуємо колекцію лисиців
    beforeEach(async () => {
        await Fox.deleteMany({});
    });

    // Тести для створення запису про нового лисиця (POST-запит)
    describe('POST /api/foxs', () => {
        it('має створити запис про нового лисиця', done => {
            // Тестові дані лисиця
            const fox = {
                name: 'Вухань',
                age: 2,
            ...
            // Виконуємо POST-запит для створення запису про лисиця
            chai.request(app)
                .post('/api/foxs')
                .send(fox)
                .end((err, res) => {
                    ...
                    expect(res.body).to.have.property('name', fox.name);
                    expect(res.body).to.have.property('age', fox.age);
                    expect(res.body).to.have.property('height', fox.height);
                    expect(res.body).to.have.property('weight', fox.weight);
                    expect(res.body).to.have.property('gender', fox.gender);
                    expect(res.body).to.have.property('description', fox.description);
    ...
    // Тести для отримання всіх записів лисиців (GET-запит)
    describe('GET /api/foxs', () => {
        it('має отримати всіх лисиців', async () => {
            // Створюємо тестовий запис лисиця
            const testFox = new Fox({
                name: 'Білан',
                age: 3,
                height: 35,
                weight: 3.2,
            ...
            // Виконуємо GET-запит для отримання всіх записів лисиців
            const res = await chai.request(app).get('/api/foxs');
    ...
    // Тести для отримання запису конкретного лисиця за ID (GET-запит)
    describe('GET /api/foxs/:id', () => {
        it('має отримати конкретного лисиця за id', async () => {
            // Створюємо запис тестового лисиця
            const testFox = new Fox({
                name: 'Косий',
                age: 1,
                height: 25,
            ...
            const savedFox = await testFox.save();

            // Виконуємо GET-запит для отримання запису лисиця за ID
            const res = await chai.request(app).get(`/api/foxs/${String(savedFox._id)}`);
            expect(res).to.have.status(200);
        ...
        it('має повернути 404 для неіснуючого лисиця', async () => {
            // Виконуємо GET-запит для неіснуючого ID лисиця
            const res = await chai.request(app).get('/api/foxs/654321654321654321654321');
            expect(res).to.have.status(404);
        });
    });

    // Тести для повного оновлення запису про лисиця (PUT-запит)
    describe('PUT /api/foxs/:id', () => {
        it('має повністю оновити запис про лисиця', async () => {
            // Створюємо тестового лисиця
            const testFox = new Fox({
                name: 'Оригінальний',
                age: 1,
                height: 25,
            ...
            // Виконуємо PUT-запит для повного оновлення запису про лисиця
            const res = await chai
                .request(app)
                .put(`/api/foxs/${String(savedFox._id)}`)
                .send(updatedData);
        ...
        it("має завершитися невдачею при відсутності обов'язкових полів", async () => {
            ...
            const unchangedFox = await Fox.findById(savedFox._id);
            expect(unchangedFox).to.have.property('name', 'Оригінальний');
            expect(unchangedFox).to.have.property('height', 25);
            expect(unchangedFox).to.have.property('weight', 1.8);
        });
    });

    // Тести для часткового оновлення запису про лисиця (PATCH-запит)
    describe('PATCH /api/foxs/:id', () => {
        it('має частково оновити запис про лисиця', async () => {
            // Створюємо тестового лисиця
            const testFox = new Fox({
                name: 'Оригінальний',
                age: 1,
            ...
            const res = await chai
                .request(app)
                .patch(`/api/foxs/${String(savedFox._id)}`)
                .send(patchData);
    ...
    describe('HEAD /api/foxs', () => {
        it('має повернути заголовки метаданих', async () => {
            // Виконуємо HEAD-запит
            const res = await chai.request(app).head('/api/foxs').set('Accept', 'application/json');
    ...
    // Тести для видалення запису лисиця (DELETE-запит)
    describe('DELETE /api/foxs/:id', () => {
        it('має видалити запис про лисиця', async () => {
            // Створюємо тестового лисиця
            const testFox = new Fox({
                name: 'Стрибунець',
                age: 2,
            ...
            const res = await chai.request(app).delete(`/api/foxs/${String(savedFox._id)}`);
            expect(res).to.have.status(200);
            expect(res.body).to.have.property('message', 'Запис про лисиця видалено');

            // Перевіряємо, що запис про лисиця дійсно видалено з бази
            const findFox = await Fox.findById(savedFox._id);
            expect(findFox).to.be.null;
        });
    });
});
